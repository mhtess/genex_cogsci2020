...
Contra the theoretical proposals, we find that in adults, a generic is worth at least two pedagogically sampled examples. 
...
\end{center}
\caption{Experiment results.  A: Experiment 1 (property was not labeled in the observation conditions). B: Experiment 2 (property was labeled in the observation conditions). Means (vertical lines) and 95\% bootstrapped confidence intervals (horizontal lines) appear above the empirical histograms. Vertical dashed lines represent the confidence interval for the \emph{generic only} condition. Data from the \emph{generic only} condition of Experiment 1 is reproduced in (B) to ease visual comparison. }
\label{fig:results}
\end{figure*}
...
\end{center}
\caption{By-item mean ratings. A: Expt.~1, participants drew stronger generalizations about the artifact than the other two items, primarily in the 1x-3x Accidental conditions. B: No comparable effect is observed in Expt.~2. Error-bars denote bootstrapped 95\% confidence intervals. Vertical dashed lines represent the confidence interval for the \emph{generic only} condition.}
...
The posterior distribution over thresholds after hearing a generic would be non-uniform and skewed towards higher values; this posterior could then become the prior for the next generic heard, which could be cashed in for more observations than just one. We leave the proof of this relationship for future work. 
...
The generics--examples exchange-rate is thus not just one-dimensional (number of examples); it should also take into account the heterogeneity and representativeness of the examples. 
...
\citeA{Cimpian2010a} compared the strength of generalization implied by a generic to the statistics of the feature (e.g., ``30\% of lorches have purple feathers'') that led participants to endorse the generic (i.e., judge ``Lorches have purple feathers'' as true), finding that generics were interpreted more strongly than what one would expect given the statistical information that yielded generic endorsement. 
\citeA{kushnir2016translating} examined the strength of generalization after hearing generic language and then observing instances with/without the property (e.g., hearing ``Blickets squeak'' and  observing 2/10 blickets squeak).
Both of these paradigms  indirectly measure the generics--examples exchange-rate.
In \citeA{Cimpian2010a}, the equivalence is derived via truth judgments of generics (i.e., at what point do people endorse generics?).
...
For example, the Accidental Example condition is not really an accident: We experimenters designed the task in order to depict an accident.
...
Language and observations are the informational backbone upon which we build our knowledge of the world. 
The exchange rate of about two or three pedagogical examples for a generic suggests that the language of generalizations can save an instructor scrambling to find a good demonstration and that a few good examples are worth about as much as anybody can describe in words.
