---
title: "How many observations is one generic worth? (CogSci 2020)"
author: "MH Tessler, Sophie Bridgers"
date: "1/31/2020"
output: pdf_document
---
...
for (data.path in data.paths){
  ...
  #    data.worker$correctId = c(NA, NA, NA, T, T, T)
      data.worker %>%
...
df.trials %>%
  ...
  count()
```
Some workers were able to do the experiment twice. Here, I grab only the data from their first submission.
...
```{r}
df.trials.filtered <- df.trials.removeDuplicateParticipants %>%
...
left_join(
  ...
  arrange(condition) %>%
  #kable() %>%
  write_csv(., path = "../writing/cogsci20/output_from_r/n_obs_bf_5000k_5000k.csv")
```
